...
class VideoRoomHandle extends Handle {
    ...
    if (plugindata && plugindata.data && plugindata.data.videoroom) {
      ...
      switch (videoroom) {
        ...
        case 'attached':
          /* Store room and feed id */
          this.room = room;
          this.feed = message_data.streams[0].feed_id; // new insert
        ...
        case 'updated':
          janode_event.data.streams = message_data.streams // Selected the first part of the stream
   ...
   */
  async configure({ feed, audio, video, data, offer_video, bitrate, record, filename, display, restart, update, streams, descriptions, sc_substream_layer, sc_substream_fallback_ms, sc_temporal_layers, e2ee, jsep }) {
    const body = {
    ...
    console.log('===========Configure data sent to the server=========', body)
    ...
    if (event === PLUGIN_EVENT.CONFIGURED && evtdata.configured === 'ok') {
      ...
      console.log('===========Configure response from the server=========', response)
      console.log('===========Configure response from the server=========', data)
      return evtdata;
    }
    const error = new Error(`unexpected response to ${body.request} request`);
    throw (error);
